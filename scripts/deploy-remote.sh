...
APP_NAME="nostr-blog"
SERVICE_NAME="nostr-blog"
INSTALL_DIR="/var/www/nostr-blog"
...
SSH_TARGET=""
SSH_LOGIN_USER=""
SSH_CONTROL_PATH="${TMPDIR:-/tmp}/nostr-blog-ssh-%C"
...
render_header() {
	...
	printf '\n'
	hr
	panel_line "$(status_info "nostr-blog deploy installer")"
	hr
	...
	panel_line "multiuser: ${MULTIUSER}"
	if [[ "$MULTIUSER" == true ]]; then
		panel_line "npubs    : ${NPUBS[*]:-<none>}"
	else
...
Options:
  ...
  --port <port>            App port on the remote host (default: 4321)
  --install-dir <path>     Remote install path (default: /var/www/nostr-blog)
...
managed_marker="# managed by nostr-blog"
...
printf '\n'
hr
panel_line "nostr-blog remote installer"
hr
panel_line "install  : ${INSTALL_DIR}"
panel_line "service  : ${SERVICE_NAME}"
panel_line "port     : ${PORT}"
...
if [[ "$MULTIUSER" == true && -n "$NPUBS_JSON" ]]; then
	panel_line "npubs    : $(printf '%s' "$NPUBS_JSON" | head -c 60)"
else
	panel_line "npub     : ${NPUB:-<setup UI>}"
fi
hr
...
SERVICEEOF
	...
	sudo_run install -m 0644 "$service_file" /etc/systemd/system/${SERVICE_NAME}.service
	rm -f "$service_file"
}
...
detect_caddy_snippet_dir() {
	...
	local dir
	if [[ -n "${NOSTR_BLOG_CADDY_SNIPPET_DIR:-}" ]]; then
		printf '%s' "$NOSTR_BLOG_CADDY_SNIPPET_DIR"
		return 0
	fi
	if [[ -f "$main_file" ]]; then
...
detect_nginx_style() {
	...
	if [[ -n "${NOSTR_BLOG_NGINX_STYLE:-}" ]]; then
		printf '%s' "$NOSTR_BLOG_NGINX_STYLE"
		return 0
	fi
...
REMOTESCRIPT
	...
	# Build NPUBS as a JSON array for multi-user
	local npubs_json=""
	if [[ "${#NPUBS[@]}" -gt 0 ]]; then
		npubs_json="["
		local first=true
		for n in "${NPUBS[@]}"; do
			$first && first=false || npubs_json+=","
			npubs_json+="\"$n\""
		done
		npubs_json+="]"
	fi
...
if [[ -z "$SSH_TARGET" ]]; then
	SSH_TARGET="${NOSTR_BLOG_SSH_TARGET:-}"
fi
if [[ -z "$SSH_TARGET" ]]; then
	usage
...
if [[ "$PORT" == "4321" && -n "${NOSTR_BLOG_PORT:-}" ]]; then
	PORT="$NOSTR_BLOG_PORT"
fi
if [[ "$INSTALL_DIR" == "/var/www/nostr-blog" && -n "${NOSTR_BLOG_INSTALL_DIR:-}" ]]; then
	INSTALL_DIR="$NOSTR_BLOG_INSTALL_DIR"
fi
if [[ "$SERVICE_USER" == "www-data" && -n "${NOSTR_BLOG_SERVICE_USER:-}" ]]; then
	SERVICE_USER="$NOSTR_BLOG_SERVICE_USER"
fi
if [[ "$SERVICE_GROUP" == "www-data" && -n "${NOSTR_BLOG_SERVICE_GROUP:-}" ]]; then
	SERVICE_GROUP="$NOSTR_BLOG_SERVICE_GROUP"
fi
if [[ "$PROXY_MODE" == "auto" && -n "${NOSTR_BLOG_PROXY:-}" ]]; then
	PROXY_MODE="$NOSTR_BLOG_PROXY"
fi
if [[ -z "$DOMAIN" ]]; then
	DOMAIN="${NOSTR_BLOG_DOMAIN:-}"
fi
...
if [[ "$DRY_RUN" == false && -n "${NOSTR_BLOG_DRY_RUN:-}" ]]; then
	is_true "$NOSTR_BLOG_DRY_RUN" && DRY_RUN=true || true
fi
if [[ "$SKIP_BUILD" == false && -n "${NOSTR_BLOG_SKIP_BUILD:-}" ]]; then
	is_true "$NOSTR_BLOG_SKIP_BUILD" && SKIP_BUILD=true || true
fi
